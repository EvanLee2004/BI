...
## 2.6.3+ 紧急 · 废止飞书外发（2026-07-25）

- **硬令**：禁止再向公司大群/「财经新闻」机器人/任何飞书 webhook 发消息（含测试）。
- `notify.py` 永久 no-op；生产 `本地配置.json` 已删除 `feishu_webhook_url`。
- 设置页去掉 webhook 输入；API 强制回空；healthcheck 不再调飞书。
- 告警仅本机日志。若将来要告警须单独告警群+书面批准。
