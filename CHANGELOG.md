...
### 看端口径脚注全员不显示
- 看端「期间费用构成」「费用明细」上方口径灰字从 DOM 删除（管理员/整体/BU 均不渲染）
- 后端 `/api/v1/vm/ledger` `caliber_note` 与 Excel「口径说明」sheet 保留；「显示全部台账记录」保留
- 守卫：`tests/test_caliber_notes_hidden_3_7_21.py`；H20 前端改为「不渲染」断言

### 未改
- profit 公式 / CIFS / 会话 TTL / 账号明文契约（MADR-0020）/ 默认期间费用口径
...
- hold 未改：明文 HTTP、看板密码明文 MADR-0020。
