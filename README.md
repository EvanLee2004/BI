...
| 端 | 地址 | 给谁 | 能干什么 |
|---|---|---|---|
| 用户端 | `/` | 管理层（只读，无需登录） | 看全部板块、切周期、导出整页图片 |
| BU 用户端 | `/bu/{token}` | 对应 BU 负责人（独立链接） | 仅看本 BU 的完整利润表与下单/回款排名；按销售人员映射过滤，跨 BU 不泄漏 |
| 用户端·按时间段看 | `/`（板块③「按时间段看」按钮） | 全员 | 选任意日期区间（或单日），三张排名卡就地按该段统计+一行合计；排名卡「其余 N 个」点开看完整排名弹窗（/api/daily 只读实时算） |
...
- 管理员默认口令 `kanban2026`（环境变量 `KANBAN_ADMIN_PW` 覆盖；密钥文件在 `数据/`，不进 git）。
