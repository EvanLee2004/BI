...
| 版本 | 架构 | 质量 |
|:---:|:---:|:---:|
| **v1.6.0-beta** | 五层单向 · 看端+管理端 static · 金额 INTEGER 分 · WAL · **Ubuntu/Windows 双部署** | 回归红线 32 周期 · 测试 **60 文件 / 602 例** · 看端明细白名单 |
...
> **图集与代码对齐说明（2026-07-16 · v1.6）**  
> 对照 `VERSION=1.6.0-beta`：五层架构 / 登录分流 / 部署拓扑（**主线 Ubuntu 22.04**，Windows legacy）/ 模块图 / 时序 / ER **均为当前代码现状**。  
> 看端费用明细列走白名单（口径人=业务员）；完整从零步骤见 `docs/Ubuntu部署手册.md` / `docs/Windows部署手册.md`（legacy）。  
