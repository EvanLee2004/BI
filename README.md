...
> **截图状态（2026-07-31 核对 · v3.6.1）**：`docs/images/ui/*` 用 `_golden_data` / 演示合成数据截取（示例客户/员工编号，**无真实客户名**）。**若未按 3.6.1 重截，图上仍是旧演示批次**（角标可能仍显示 v2.3.x / 旧板块顺序）。**现网代码布局（3.6.1）** 首屏六段：一基本情况 → 二下单与回款 → 三重点客户下单情况追踪 → 四经营利润 → 五收入与毛利结构 → 六费用明细。截图区**待 3.6.1 重截**后再更新图文件；在此之前请以 `VERSION` + 用户手册板块表为准。**禁止**把含真实金额/客户名的截图提交进 git。
