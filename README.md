...
>
> 开发号 **v8.7.2** · 产品版本 **v1.0（公测 Beta）**·周一上线版 · 测试 **302** 全绿 · 回归红线 32 周期一分不差
> 2026-07-12 批次：产品版本号+更新日志页(v8.4) · 多次更新时间(v8.5) · 账号可绑多个 BU(v8.6) · 一键更新+看门狗(v8.7) · 对标 Gitee(v8.7.1)
> 2026-07-13：产品升 **公测 Beta**（VERSION=1.0-beta）+ code review 修 #1（收单月份越界校验，体检不再静默漏计）
...
**产品版本号 + 一键更新（v8.4/v8.7）**：管理端设置页有「版本与更新日志」卡——**产品版本号**（0.9 试运行 → **1.0-beta 公测 Beta**（现·周一上线）→ 去 -beta 升 1.0 正式版，与 git 开发号 v8.x 分开）+ 大白话更新日志。**一键更新**：点「检查更新」比对代码仓库（`git fetch`），有新版本可「一键更新并重启」——安全快进拉取（`git pull --ff-only`，护栏：工作区脏/分叉/非仓库拒绝）+ 看门狗按退出码 42 用新代码自动重启。需用 `看门狗启动.bat` 起服务。
...
- 版本节点：… → v8.0 账号与 BU 解耦 → v8.1 销售归属自助 + 配置留痕 → v8.3 板块③收入与毛利结构 → v8.4 产品版本号/更新日志页 → v8.5 多次更新时间 → v8.6 账号绑多个 BU → v8.7 一键更新+看门狗 → v8.7.1 对标 Gitee → **v8.7.2 产品升公测 Beta + code review 修 #1**（完整 CHANGELOG 在项目本地文档库）。
