...
| 当前版本 | 技术栈 | 生产形态 |
|:---:|:---|:---|
| **v2.5.0**（以根目录 [`VERSION`](./VERSION) 为准） | Python · SQLite · FastAPI · Vue 3 · ECharts | 公司 Ubuntu · nginx · systemd · 定时刷新 |

> 版本历史见 [`CHANGELOG.md`](./CHANGELOG.md)。业务数据与账号密码**不进本仓库**。  
> **2.5.0 要点**：**全员同一登录页** `/login`，按账号分流（管理员→后台 / 整体→公司看板 / 业务线→本 BU）；已删除独立「管理员端登录」门面。  
...
> **截图**：`docs/images/ui/*` 用 `_golden_data` 合成数据截取（示例客户/员工编号，无真实客户名）；部分图摄于 2.3.x 主题批次，界面主结构与现网一致。本地三主题全貌可另存 gitignore 目录自审。**禁止**把含真实金额/客户名的截图提交进 git。
