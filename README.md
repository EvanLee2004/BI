...
>
> 开发号 **v8.7.4** · 产品版本 **v1.0（公测 Beta）**·周一上线版 · 测试 **308** 全绿 · 回归红线 32 周期一分不差
> 2026-07-12 批次：产品版本号+更新日志页(v8.4) · 多次更新时间(v8.5) · 账号可绑多个 BU(v8.6) · 一键更新+看门狗(v8.7) · 对标 Gitee(v8.7.1)
> 2026-07-13：产品升 **公测 Beta**(v1.0-beta) + code review 修 #1(收单月份越界) + 一键更新补依赖自动安装&坏版本自动回滚(v8.7.3) + **配置分离修 F-01**(v8.7.4·台账路径进设置页·部署机 config.json 永不脏→一键更新可用)
...
**产品版本号 + 一键更新（v8.4/v8.7）**：管理端设置页有「版本与更新日志」卡——**产品版本号**（0.9 试运行 → **1.0-beta 公测 Beta**（现·周一上线）→ 去 -beta 升 1.0 正式版，与 git 开发号 v8.x 分开）+ 大白话更新日志。**一键更新**：点「检查更新」比对代码仓库（`git fetch`），有新版本可「一键更新并重启」——安全快进拉取（`git pull --ff-only`，护栏：工作区脏/分叉/非仓库拒绝）→ **依赖变了自动 `pip install`**（默认走国内清华 PyPI 镜像 `config.pip_mirror`，部署机免翻墙；装失败→回滚拉取不重启）→ 看门狗按退出码 42 用新代码自动重启；**坏版本自愈**：更新后启动即崩→看门狗据回滚点标记自动回滚上一版本一次。需用 `看门狗启动.bat` 起服务。
...
- 版本节点：… → v8.4 产品版本号/更新日志页 → v8.5 多次更新时间 → v8.6 账号绑多个 BU → v8.7 一键更新+看门狗 → v8.7.1 对标 Gitee → v8.7.2 产品升公测 Beta + code review 修 #1 → **v8.7.3 一键更新补依赖自动安装+坏版本自动回滚**（完整 CHANGELOG 在项目本地文档库）。
- 产品版本阶段：0.9 试运行 → **1.0-beta 公测 Beta（周一上线）** → 1.0 正式版（去 -beta）。
