...
> 一套**轻量自建 BI**：每天自动从业务系统抓数 + 读财务台账，算到**税前利润**，出科技风暗色（可切浅色）、手机可看的驾驶舱；管理员可在网页上改数、手填、看历史。零重型 BI 依赖——Python + SQLite + FastAPI + 手写 SVG。**v1.4**：CSS/JS 外置 `static/` + `/api/v1/cockpit` JSON；**无 React/npm**；页面 HTML 仍由原 render 生成（观感对齐 v1.3.1）。
>
> 版本 **v1.4.0-beta（公测 Beta）** · 回归红线 32 周期 · golden 数字全等
> （v8.9.1 起产品版本号即唯一版本号，旧「开发号 v8.x」并入 v1.x-beta 单一体系；VERSION 文件为唯一源）
> 2026-07-12：产品版本号/更新日志页(v8.4) · 多次更新时间(v8.5) · 账号可绑多个 BU(v8.6) · 一键更新+看门狗(v8.7) · 对标 Gitee(v8.7.1)
> 2026-07-13：升 **公测 Beta**(v1.0-beta) + code review 修#1(收单月份越界) + 一键更新补依赖自动安装&坏版本自动回滚(v8.7.3) + **配置分离修 F-01**(v8.7.4·台账路径进设置页·部署机 config.json 永不脏→一键更新可用) + 智云连接内置默认(v8.7.5) + 首部署引导页修 F-02(v8.7.6)
> 2026-07-14：更新诚实化/设置统一保存(v1.0.4) · pip 走国内镜像(v1.0.5) · 回款板块下移+**公共费用按月分摊到 BU**(v1.0.6/v1.1.0) · 回款卡月份高亮+台账归属校验(v1.1.1) · **陆总0714过盘反馈全量落地**(迭代22·v1.2.0：系统成本率/分摊沿用最近填写月/销售利润率/下单按BU/BU页对齐整体页) · 手填清单config注入(v1.2.1) · **看端领导向精简+KPI扫读+回款排名全站按BU+管理端亮色主题**(v1.2.2)
...
2. **经营利润**：收入成本毛利率月度组合图 → 管理利润表（点大类→右侧抽屉看构成，两级下钻到费用明细细类）→ 期间费用构成（按大类环形图｜按类别｜按业务BU 三态切换，每条可点开细项）→ 回款按月+回款下单率+年预算完成率。
3. **收入与毛利结构**（v8.3）：收入·按客户 / 收入·按销售 两卡（对称两列/手机单列），每行 收入+毛利率，卡头集中度「前5大占收入%」；**「其余 N 个」点开→全量弹窗**（/api/profit_ranking，须整体/管理员会话）；**长客户名悬浮显全名**（#tip 即时浮层）。**确认口径**（按整单交付日期归属）：收入=交付额÷1.06、毛利=收入−项目成本（**项目直接毛利**，未含内部译员/手填调整，与利润表总毛利略有差异，口径写在 footer）。
4. **下单与回款排名**：下单按部门/按销售、回款按客户（期内前 10 + 其余合计）；日期条常显跟顶、可查任意起止段、「其余」点开全量弹窗。
...
**账号与 BU 分页（v8.0·v8.6 多 BU）**：登录账号在 `数据/看板账号.json`（权限=管理员/整体/**按 BU**；明文密码仅管理员可见；看的人可自改）。**一个账号可绑定一组 BU**（v8.6：整体=全部；也能只给某几个 BU 的子集；旧"单个 BU 名"账号完全兼容）；多 BU 账号登录后落在第一个绑定 BU 页、顶部「我的 BU」切换条**只列其绑定的 BU、绝不泄漏他 BU**。BU 数据归属在 `数据/BU配置.json`（BU 名+负责人备注+**销售名单=子页数字口径**，无密码）。两文件均不进 git；样例见 `docs/看板账号样例.json`、`docs/BU配置样例.json`。

**产品版本号 + 一键更新（v8.4/v8.7）**：管理端设置页有「版本与更新日志」卡——**产品版本号**（0.9 试运行 → **公测 Beta**（现 `1.2.2-beta`）→ 去 -beta 升 1.0 正式版；读根目录 `VERSION` 文件为唯一源）+ 大白话更新日志。**一键更新**：点「检查更新」比对代码仓库（`git fetch`），有新版本可「一键更新并重启」——安全快进拉取（`git pull --ff-only`，护栏：工作区脏/分叉/非仓库拒绝）→ **依赖变了自动 `pip install`**（默认走国内清华 PyPI 镜像 `config.pip_mirror`，部署机免翻墙；装失败→回滚拉取不重启）→ 看门狗按退出码 42 用新代码自动重启；**坏版本自愈**：更新后启动即崩→看门狗据回滚点标记自动回滚上一版本一次。需用 `看门狗启动.bat` 起服务。

**多次更新时间（v8.5）**：每日自动更新从单个固定时间→可增删多个时间点（如 09:30 / 12:00 / 17:30），各到点各跑一次；Windows 每个时间点一个计划任务（主任务 + `_2.._n`）。
...
# 数字与外部系统
GET /api/v1/cockpit          # JSON（含 numbers）
GET /api/v1/cockpit/view     # 像素级 HTML
docs/api-v1-cockpit.md
golden/                      # 第0步金标准与功能清单
```

回退「整页内联 HTML」：`KANBAN_LEGACY_INLINE=1 python run.py --serve`
...
KANBAN_OFFLINE=1 sh tests/run_verify.sh   # 一键验证：语法+端到端+回归红线+297测试（离线，不碰网络）
```
...
src/
  ...
  accounts.py         看板账号表（权限=管理员/整体/BU；可绑多 BU=可见BU 列表；明文密码）
  bu.py               BU 数据归属读写、校验（BU 名+销售名单+分摊比例；真实配置仅在 数据/）
  version.py          产品版本号（读根目录 VERSION）+ 面向用户更新日志
  updater.py          一键更新（git fetch 比对 / pull --ff-only / 退出码 42 触发看门狗重启）
...
tests/                297 用例（19 套）+ 回归红线 + run_verify.sh 一键验证
docs/                 部署手册 / 数据来源说明 / 设计图
看门狗启动.bat        部署起服务（循环跑 --serve，支持一键更新后自动重启）
```
...
- 版本节点：… → v8.7 一键更新+看门狗 → v8.7.1 对标 Gitee → v8.7.2 升公测 Beta+review#1 → v8.7.3 依赖自动安装+坏版本回滚 → v8.7.4 配置分离修 F-01 → v8.7.5 智云连接内置默认 → v8.7.6 首部署引导页修 F-02 → v1.0.4 更新诚实化/设置统一保存 → v1.0.5 pip 国内镜像 → v1.0.6/v1.1.0 回款下移+公共费用按月分摊 → v1.1.1 回款月份高亮+归属校验 → **v1.2.0 陆总0714过盘反馈全量落地**（迭代22）→ v1.2.1 手填清单config注入 → **v1.2.2 看端领导向精简+KPI扫读+回款排名全站按BU+管理端亮色主题**（完整 CHANGELOG 在项目本地文档库）。
- 产品版本阶段：0.9 试运行 → **1.x-beta 公测 Beta（现 `1.2.2-beta`）** → 1.0 正式版（去 -beta）。
...
> **迭代20（2026-07-14）**：公共费用分摊改**按月比例**，在管理端「数据调整→人工填写」填（先显示本月公共费用总额）；合计可<100%（剩余留公司层，有提示）；BU 页与全公司费用构成视图同一套分摊结果、总额不变。看板板块④改「资金与回款」（回款情况+排名）。
