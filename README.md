...
>
> 当前版本 **v7.3.1**（2026-07-11）· 测试 **132** 全绿 · 回归红线 32 周期一分不差
...
KANBAN_OFFLINE=1 sh tests/run_verify.sh   # 一键验证：语法+端到端+回归红线+132测试（离线，不碰网络）
```
...
tests/                132 用例（9 套）+ 回归红线 + run_verify.sh 一键验证
...
- 版本节点：v6 驾驶舱重构 → v7 数据库+双端改造 → v7.2 预算与费用视角 → v7.3 自动抓数+设置页+月区间+排名+历史快照+图片导出 → v7.3.1 安全与健壮性修复（完整 CHANGELOG 在项目本地文档库）。
