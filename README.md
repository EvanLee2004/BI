...
| 管理员端 | `/admin` | 财务（密码会话） | 改明细数（留痕可撤销）、手填月度数据、维护年度预算、复核台账/调整/历史快照、系统设置 |
...
**管理员端（/admin）**：数据总览 / 明细台账（查+改值剔除，留痕可撤销）/ 手填与调整（月度表单）/ 年度预算维护 / 复核（台账、未填分类、调整台账、**历史快照**——年月日级联回看任意一天的页面原样）/ 设置（每日自动更新时间、备份保留天数、智云账号密码——换账号只填两样立即生效）。
