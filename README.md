...
>
> 当前版本 **v7.7**（2026-07-11）· 测试 **182** 全绿 · 回归红线 32 周期一分不差
...
| 端 | 地址 | 给谁 | 能干什么 |
|---|---|---|---|
| 用户端 | `/` | 管理层（只读，密码登录·初始 8888） | 看全部板块、切周期、导出整页图片、右上「🔑密码」改自己的登录密码 |
| BU 用户端 | `/bu/{token}` | 对应 BU 负责人（独立链接+密码·初始 8888） | 仅看本 BU 的完整利润表与下单/回款排名；按销售人员映射过滤，跨 BU 不泄漏；可改本页密码。整体页/管理员会话可看任意 BU |
| 用户端·按时间段看 | `/`（板块③「按时间段看」按钮） | 已登录整体页的人 | 选任意日期区间（或单日），三张排名卡就地按该段统计+一行合计；排名卡「其余 N 个」点开看完整排名弹窗（/api/daily 只读实时算，须整体页/管理员会话） |
| 管理员端 | `/admin` | 财务（密码会话） | 改明细数（留痕可撤销）、手填月度数据、维护年度预算、异常处理（总览/调整台账/下单未填部门/费用未分类/历史快照）、系统设置 |
...
**管理员端（/admin）**：数据总览 / 明细台账（查+改值剔除，留痕可撤销）/ 手填与调整（月度表单）/ 年度预算维护 / 异常处理（**总览**计数卡分诊台、调整台账、**下单未填部门**——行内选部门即归类写调整、费用未分类（台账）、**历史快照**——年月日级联回看任意一天的页面原样）/ 设置（每日自动更新时间、备份保留天数、智云账号密码——换账号只填两样立即生效）。

**BU 分页（/bu/{token}）**：管理员在「设置 → BU 配置」维护 BU、负责人和销售名单，系统为每个 BU 生成不可猜的独立链接。页面只从该 BU 的销售数据构建；公共费用暂不分摊、手填项待陆总确认填法，均明确标注而不伪装为零。配置文件 `数据/BU配置.json` 含真实姓名与链接，永不提交 Git；初版销售映射仍待陆总确认。
...
KANBAN_OFFLINE=1 sh tests/run_verify.sh   # 一键验证：语法+端到端+回归红线+184测试（离线，不碰网络）
```

- 管理员默认口令 `kanban2026`（环境变量 `KANBAN_ADMIN_PW` 覆盖；密钥文件在 `数据/`，不进 git）；管理员端「设置→登录密码」可改。
- **全看板密码制（v7.8）**：整体页与各 BU 页初始密码都是 `8888`，看的人登录后点页面右上「🔑密码」自己改（整体/各 BU/管理员三处各自独立、改完立即生效并持久化）。
...
src/
  ...
  bu.py               BU 配置读写、校验与独立链接 token 生成（真实配置仅在 数据/）
...
tests/                184 用例（12 套）+ 回归红线 + run_verify.sh 一键验证
...
- 开发铁律 11 条（智云 xlsx 绝不 read_only / 列按表头找 / 必需列缺失即报错 / 同名控件合并去重空不覆盖有值 / 自由文本进 HTML 必转义……）全文见 [CLAUDE.md](CLAUDE.md)。
- 每个改动批次：补测试 → `run_verify.sh` 全绿 → 同步 CHANGELOG/测试说明 → commit+push。

## 版本与发布安全

- 版本节点：v6 驾驶舱重构 → v7 数据库+双端改造 → v7.2 预算与费用视角 → v7.3 自动抓数+设置页+月区间+排名+历史快照+图片导出 → v7.3.1 安全与健壮性修复 → v7.3.2 过期疑似批量处理 → v7.7 按销售归属的 BU 独立分页（完整 CHANGELOG 在项目本地文档库）。
