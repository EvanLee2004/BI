...
>
> 当前版本 **v7.9**（2026-07-11）· 测试 **196** 全绿 · 回归红线 32 周期一分不差
...
| 端 | 地址 | 给谁 | 能干什么 |
|---|---|---|---|
| 用户端 | `/` | 账号「整体」=姜总/陆总（只读） | 账号+密码登录后看全部板块、顶部可点进各 BU 页、切周期、导出整页图片 |
| BU 用户端 | `/`（同一入口·账号=BU 名） | 各 BU 负责人 | 登录后首页直接是本 BU 页（完整利润表+下单/回款排名，按销售名单过滤、跨 BU 不泄漏）；整体/管理员可看任意 /bu/{BU名} |
...
**BU 分页（账号制）**：管理员在「设置 → BU 配置」维护 BU、负责人、销售名单（数据归属）与各账号密码。BU 名即登录账号，负责人在看板首页登录后直接看到本 BU 页；「整体」账号看全部并可点进各 BU 页（/bu/{BU名}）。页面只从该 BU 的销售数据构建；公共费用暂不分摊、手填项待陆总确认填法，均明确标注而不伪装为零。配置文件 `数据/BU配置.json` 含真实姓名与口令哈希，永不提交 Git；初版销售映射仍待陆总确认。
...
- **账号制（v7.9）**：看板一个入口 `/`，账号「整体」或 BU 名 + 密码（初始 `8888`）分流；**密码由管理员集中管理**（设置页：整体密码/各 BU 行内密码/管理员口令），看的人不能自己改。
...
- 版本节点：v6 驾驶舱重构 → v7 数据库+双端改造 → v7.2 预算与费用视角 → v7.3 自动抓数+设置页+月区间+排名+历史快照+图片导出 → v7.3.1 安全与健壮性修复 → v7.3.2 过期疑似批量处理 → v7.7 按销售归属的 BU 独立分页 → v7.9 看板账号制（一个入口按账号分流·密码集中管理）（完整 CHANGELOG 在项目本地文档库）。
