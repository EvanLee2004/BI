...
> **截图状态（2026-07-29 核对）**：`docs/images/ui/*` 用 `_golden_data` 合成数据截取（示例客户/员工编号，无真实客户名）；**摄于 2.3.1 演示批次**（图上角标可能仍显示 v2.3.x）。**主布局**（五 KPI、利润区、结构区、三主题霓虹/深空）与现网一致；**尚未重截**的细节差：管理端顶栏「用户统计」、KPI 含税/不含税方案 A 小字、设置页无飞书告警卡、晨光主题整页。本地三主题全貌可另存 gitignore 目录自审。**禁止**把含真实金额/客户名的截图提交进 git。
