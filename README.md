...
>
> 当前版本 **v7.3.2**（2026-07-11）· 测试 **136** 全绿 · 回归红线 32 周期一分不差
...
⑤ 展示层   render：预渲染双端 HTML；server：FastAPI 提供页面与 /api/*
```
...
KANBAN_OFFLINE=1 sh tests/run_verify.sh   # 一键验证：语法+端到端+回归红线+136测试（离线，不碰网络）
```
...
tests/                136 用例（9 套）+ 回归红线 + run_verify.sh 一键验证
...
- 开发铁律 10 条（智云 xlsx 绝不 read_only / 列按表头找 / 必需列缺失即报错 / 同名控件合并去重空不覆盖有值 / 自由文本进 HTML 必转义……）全文见 [CLAUDE.md](CLAUDE.md)。
- 每个改动批次：补测试 → `run_verify.sh` 全绿 → 同步 CHANGELOG/测试说明 → commit+push。

## 版本与发布安全

- 版本节点：v6 驾驶舱重构 → v7 数据库+双端改造 → v7.2 预算与费用视角 → v7.3 自动抓数+设置页+月区间+排名+历史快照+图片导出 → v7.3.1 安全与健壮性修复 → v7.3.2 过期疑似批量处理（一键听源头/逐条坚持我的数）（完整 CHANGELOG 在项目本地文档库）。
