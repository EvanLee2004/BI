...
**账号与 BU 分页（v8.0）**：登录账号在 `数据/看板账号.json`（权限=管理员/整体/某 BU 名；明文密码仅管理员可见；看的人可自改）。BU 数据归属在 `数据/BU配置.json`（BU 名+负责人备注+销售名单，无密码）。整体账号看全部并可点进各 BU 页；BU 账号只看本 BU。两文件均不进 git；样例见 `docs/看板账号样例.json`、`docs/BU配置样例.json`。
## 快速开始
