...
| 版本 | 架构 | 质量 |
|:---:|:---:|:---:|
| **v1.6.0-beta** | 五层单向 · 看端+管理端 static · 金额 INTEGER 分 · WAL · **nginx 双模 + 存储收敛** | 回归红线 32 周期 · 看端明细白名单 · 飞书告警可选 |
...
> **图集与代码对齐说明（2026-07-16 · v1.6 · 任务书43）**  
> 对照 `VERSION=1.6.0-beta`：五层架构 / 登录分流 / 部署拓扑（**Ubuntu 主线：nginx :80 + API 127.0.0.1:8018**；**直连模式** `server_host=0.0.0.0` + `serve_static=true` 仍可用）/ 模块图 / 时序 / ER **均为当前代码现状**。  
> 看端费用明细列走白名单（口径人=业务员）；SQL 只在 `db`/`db_write`/`schema`；飞书 webhook 与审计归档在管理端「设置」。  
> 手册：`docs/Ubuntu部署手册.md`（含 nginx/ufw/fail2ban）· `docs/Windows部署手册.md`（legacy）。  
> **看端** `static/css|js` + **管理端** `static/admin/` 均已外置。矢量源在 `docs/设计图/`；README 内嵌 **PNG**（GitHub / Gitee 均可渲染）。  
> 交付说明：[v1.5 管理端分离](docs/v1.5管理端前后端分离交付说明.md) · [v1.4 看端分离](docs/v1.4前后端分离交付说明.md)
...
⑤ 展示    FastAPI · 账号分流 · static/（看端）+ static/admin/（管理端）· /api/v1 JSON
```
...
static/shell.html          登录后壳 → fetch fragments + page.js 组装
static/js/assemble/        看端组装器（零金额运算）

GET /api/v1/cockpit            驾驶舱 JSON（numbers ≡ golden）
GET /api/v1/cockpit/fragments  渲染就绪碎片（B 默认；整页 SSR view 已删）
GET /api/v1/cockpit/bu/{name}  单 BU JSON
```

说明：[交付说明](docs/v1.4前后端分离交付说明.md) · [API 契约](docs/api-v1-cockpit.md) · [渲染合同](docs/cockpit_render_contract_v1.md)

v1.5+ 看端固定 `static/shell.html` + fragments 组装（unittest 可 `SERVE_SHELL=False` 直出 HTML 便于断言）。紧急回退请用 git。
