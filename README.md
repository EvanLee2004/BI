...
3. **收入与毛利结构**（v8.3）：收入·按客户 / 收入·按销售 两卡（对称两列/手机单列），每行 收入+毛利率，卡头集中度「前5大占收入%」，长尾「其余 N 个」合计。**确认口径**（按整单交付日期归属）：收入=交付额÷1.06、毛利=收入−项目成本（**项目直接毛利**，未含内部译员/手填调整，与利润表总毛利略有差异，口径写在 footer）。
