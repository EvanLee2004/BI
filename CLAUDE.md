...
| `updater.py` | **一键更新 + 看门狗**（v8.7）：`check_update`(git fetch+比对 origin/分支)/`apply_update`(护栏→`git pull --ff-only`)/`request_restart`(退出码 `RESTART_EXIT_CODE=42`)；护栏=只 ff、脏/分叉/非仓库拒绝；配 `看门狗启动.bat`(码 42 重启) |
