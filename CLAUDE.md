...
| `updater.py` | **一键更新 + 看门狗**（v8.7·v8.7.1 可选远端）：`check_update`/`apply_update`(带 `remote` 参数，默认 `config.update_remote`→默认 origin；护栏→`git pull --ff-only <remote>`)/`request_restart`(退出码 `RESTART_EXIT_CODE=42`)；护栏=只 ff、脏/分叉/非仓库拒绝；配 `看门狗启动.bat`(码 42 重启)。部署从 Gitee clone 则 origin=Gitee 默认对标 Gitee |
...
18. **一键更新只认 fast-forward + 护栏（v8.7）**：`updater.apply_update` 只跑 `git pull --ff-only`（绝不合并/变基/产生冲突）；工作区脏、本地与远端分叉（有远端没有的提交）、非 git 仓库→一律拒绝并提示人工，绝不强拉覆盖。重启靠看门狗——服务进程拉取成功后以退出码 `RESTART_EXIT_CODE=42` 退出，`看门狗启动.bat` 据码用新代码重拉起（这个 42 两处必须一致）。改更新逻辑别绕过护栏、别在无看门狗时指望自动重启。
