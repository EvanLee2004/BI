...
- **未完成**：阶段1 其余逐卡/亮色/375/BU、阶段2 其余交互、阶段3 减重；回款月均预算线+累计摘要条**需后端填 VM 空字段**（`receipts_budget`/`summary_by_period`）；**费用月度图型面积 vs 折线待明昊拍板**。全绿 EXIT:0，tag `stage54p3` 未 push。交付报告 `docs/20260718_任务书54.3交付报告.md`。

## 当前状态（2026-07-17 · 任务书50）

- **任务书50 真组件化 + ECharts + nginx 标准架构 + 登录回退（2026-07-17）**：删 SSO；明文密码+👁+改密踢会话；Vue `v-html` 清零（结构化 VM + 真事件）；`/api/v1/vm/ledger` 任何会话白名单；图表轴/中心后端串；nginx 发 dist 为生产标准（MADR-0009）；VERSION 2.0.0-beta。交付 `docs/20260717_任务书50交付报告.md`。**main 只 commit 不 push**；32 周期数字红线不变。
- **铁律延伸（任务书50）**：① frontend/src 禁止 `v-html`（自由文本走文本插值/textContent）；② 看端明细任何会话（含管理员）一律白名单列，管理端数据调整接口另计；③ 验证只认真实退出码（铁律20）。

## 当前状态（2026-07-16 · 历史）


- **任务书39 看端布局统一与联动（2026-07-16·明昊二轮走查）**：A 回款图整行+图下摘要条；B 按时间段上移+全局周期联动；C 查询双血条（`/api/daily.dual_rankings` + BU `/api/bu_daily`）；D 费用明细胶囊/sticky/分页/年·月联动；E 费用月度堆叠柱 overall/BU（工资默隐并入其他）；F 黄横幅仅管理端；G 进度>999% 钳制。测试 `test_task39`。交付 `docs/20260716_任务书39交付报告.md`。**main 只 commit 不 push**；32 周期数字红线不变。
- **任务书37 功能与 UI 收口（2026-07-16·明昊走查）**：A1–A6 UI（去刷新提示/双血条图例/50/50/周期标注/sticky/H1 行序）+ B7 六表列筛（`db.query_detail` filters + 管理端表头）+ B8 整体费用明细与工资默隐（`overall_see_salary` 覆盖层）+ B9/B10 抓数降级黄横幅与未来日体检。测试 `test_task37_*.py`。交付 `docs/20260716_任务书37交付报告.md`。
- **任务书33 数据库大修 + 隐患清扫（2026-07-16）**：A1 原子重建 · A2 WAL · A3 金额分（SCHEMA v3：adj 分文本 + 预算比率百分位点）· A4 重复定位键 · A5 10 万行基准 · A6 备份恢复 · A7 quick_check · B 吞异常/config · C 文档。**前端/口径/golden 零变化**。交付报告 `docs/20260716_任务书33交付报告.md`。规模基线 `docs/bench_scale_100k_latest.json`。
