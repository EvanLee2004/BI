...
```
- **Linux 部署（主线）**：`docs/Ubuntu部署手册.md`；`systemctl enable --now kanban`；`bash deploy/linux/register_schedule.sh`。
- **Windows 部署（legacy）**：`看门狗启动.bat` + `注册每日更新.bat`；见 `docs/Windows部署手册.md`。
...
| `updater.py` | **一键更新 + 看门狗**：`check_update`/`apply_update`→ff-only→依赖同步→`.update_rollback`；退出码 42；Linux=`deploy/linux/start_with_rollback.sh`，Windows=`看门狗启动.bat`（legacy） |
...
2c. **平台分支集中**：定时同步走 `server.sync_schedule`（win32 schtasks / linux cron 哨兵）；Windows `.bat` 保留不删（legacy）。不引 Docker。
