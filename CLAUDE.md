...
| `bu.py` | BU 配置读写/校验 + 独立链接 token 服务端生成（真实配置只在 `数据/BU配置.json`）|
| `render.py` | 整体用户端与 BU 独立页 HTML | `charts/theme/assets` 渲染件 | `export_book` 导出 |
| `server.py` | FastAPI 三类出口：整体`/`只读、BU `/bu/{token}`严格隔离、管理员`/admin`密码会话（顶层四区=看/改数据/**异常处理**(总览/调整台账/下单未填部门/费用未分类（台账）/历史快照)/设置）、`/api/*` 编辑接口、控制台HTML(`_ADMIN_CONSOLE`) |
...
12. **BU 页严格隔离**：BU token 只能服务端生成/保存，错 token 一律 404；BU 渲染只可吃该 BU 已过滤的 summary，禁止接入整体页动态出口（`/api/daily`、`/export.png` 等）。新增 BU 字段或出口必须补“他 BU 名称/人员/客户不泄漏”测试。
...
- **v7.8 全看板账号密码制已完成**：整体页 `/` 与各 BU 页都要密码（初始 `8888`；哈希分存 管理员密钥.json viewer_* / BU配置.json 密码hash），未登录出登录页；查看会话=cookie `kanban_view`（与管理员同 HMAC 机制）；BU 会话只开本 BU，整体页/管理员会话看任意 BU；页面右上「🔑密码」自改密码（`/api/passwd` 验旧设新持久化），管理员端「设置→登录密码」改管理员口令（`/api/admin/passwd`）；**`/api/daily`+`/export.png` 收进整体页/管理员会话闸**（BU 会话 401，关掉绕过页面隔离的口子）；保存 BU 配置按 token 保留各 BU 已改密码、换链接=密码回初始。测试 **196** 全绿+回归红线。
