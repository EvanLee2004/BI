...
## 当前状态（2.2.6 · 2026-07-21）

- **版本**：`VERSION` / `src/version.py` = **2.2.6**（下单未填部门：表头筛选后本页批量归类 + 确认「仅当前页」笔数/金额；`POST /api/adjust/batch` 一次 recompute）。其上 2.2.5=翻页/展示/logo；2.2.4=十项。
- **展示端**：顶栏左=logo+标题+时间；右=版本号+主题+导出+退出；2.2.4 费用/回款/毛利率等能力保留。
