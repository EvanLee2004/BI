...
- 分支 `main`（原 gh-clean，2026-07-09 改名+设为 GitHub 默认分支；旧 master→archive-…-禁推）；tag **v7.0** = 刀0–4 数据层+双端改造完成。**v7.1 可见层已全部做完**：A组(KPI环比+迷你趋势/回款下单率两处/周期下拉 `804315b`)、B组(控制台三区重排/明细无限滚动 `c94ef2f`)、C组(老板端移除体检+banner、极淡小字兜底、体检点开看为啥 `0a8e894`)、前端科技大屏改造(抽屉下钻+FUI视觉+动效 `e99be88`·**已push公开仓库**)。回归红线一分不差、58 测试全绿。
- **2026-07-09 口径变化**：「未填分类」页改**只读清单**、去掉当场补分类（`1d2130f`·未push）——分类只在源头收单台账填，未填不计入费用、源头补填后下次更新自动计入；报表大类唯一计算用途=`compute_ledger_expenses` 的 5+3 大类白名单路由。
- 数据是**测试造数**（真客户名+假金额，7×复制），非真实经营数——不能拿数字给陆总看（阶段汇报HTML里截图已标注测试数据）。
- **v7.2（2026-07-09 深夜）**：年度预算手填（全公司下单/回款→回款图预算线+完成率）+ 费用构成三态视角（大类|部门|利润中心，横条+抽屉，守恒锁死）+ 部门费用预算执行卡（超支红）；台账已换真实数据（仅本机）；清洗层补`预算归属部门`软列。测试 94 全绿。
- **下一步**：待陆总回复智云账号 → 启动二期自动抓数（阶段汇报件见 `../../方案与文档/给陆总的汇报件/20260709_利润看板阶段汇报_给陆总.html`）；随后部署上线、接真实数据。
- ⚠ 工作区有一批 render/theme 未提交改动（删磁力流体/扫描线，另一会话在做的动效收敛），处理前先问明昊。
