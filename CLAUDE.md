...
- 分支 `gh-clean`（本地 master 永不推）；tag **v7.0** = 刀0–4 数据层+双端改造完成。**v7.1 可见层已全部做完**：A组(KPI环比+迷你趋势/回款下单率两处/周期下拉 `804315b`)、B组(控制台三区重排/明细无限滚动 `c94ef2f`)、C组(老板端移除体检+banner、极淡小字兜底、体检点开看为啥 `0a8e894`)、前端科技大屏改造(抽屉下钻+FUI视觉+动效 `e99be88`·**已push公开仓库**)。回归红线一分不差、58 测试全绿。
