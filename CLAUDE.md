...
给管理层看的**经营利润驾驶舱**：读数据源 → 算到税前利润 → 出自包含 HTML。5 用户内网/手机看，分级=账号分流（看板一个入口 `/`：「整体」账号看全部、BU 账号只看本 BU；管理员端 `/admin` 可改数）。
...
| `bu.py` | BU 配置读写/校验 + 口令哈希（BU 名即登录账号、「整体」账号保留字；真实配置只在 `数据/BU配置.json`）|
| `render.py` | 整体用户端与 BU 独立页 HTML | `charts/theme/assets` 渲染件 | `export_book` 导出 |
| `server.py` | FastAPI 出口：`/`=看板统一入口（账号分流：整体/BU 名）、`/bu/{BU名}`（整体/管理员/本 BU 会话）、管理员`/admin`密码会话（顶层四区=看/改数据/**异常处理**(总览/调整台账/下单未填部门/费用未分类（台账）/历史快照)/设置）、`/api/*` 编辑接口、控制台HTML(`_ADMIN_CONSOLE`) |
...
4. **每改必补测试 + 跑 `run_verify.sh` 全绿**（现有 test_cockpit/datalayer/adjust/server/admin_edit/fetch_zhiyun/budget/expense_views/bugfix_0711）。
...
12. **BU 页严格隔离**：BU 渲染只可吃该 BU 已过滤的 summary，禁止接入整体页动态出口（`/api/daily`、`/export.png` 等——它们只认整体/管理员会话）；未知 BU 名一律 404；账号密码只由管理员集中管理（口令哈希不下发前端）。新增 BU 字段或出口必须补“他 BU 名称/人员/客户不泄漏”测试。
...
- **v7.9 看板账号制已完成（当天推翻 v7.8 token 链接制重构）**：看的人一个入口 `/`——账号「整体」（=bu.MAIN_ACCOUNT，姜征/陆总）→ 整体页（顶部带 BU 分页入口条）；账号=BU 名 → 首页直接出本 BU 页；`/bu/{BU名}` 供整体/管理员点看（本 BU 账号也可），未知名 404；token 机制整套移除。**密码集中管理**（看的人不能自己改）：初始都 `8888`，「设置→登录密码」设整体密码（/api/viewer_passwd）+ 改管理员口令，BU 密码在 BU 配置表行内设（填=重置留空=不变，随 /api/bu_config 落盘）；BU 名不能叫「整体」。沿用：查看会话 cookie kanban_view、/api/daily+/export.png 整体/管理员会话闸（BU 会话 401）、中文密码 bytes 比较、/api/bu_config 不下发密码哈希。测试 **196** 全绿+回归红线。
...
- **v7.3.2 过期疑似批量处理（2026-07-11）**：调整台账页=「只看过期疑似」筛选+**一键听源头新值**（批量撤销，前端确认条两步：点按钮→确认保存）+**坚持我的数**（逐条 rearm：原值刷成源头现值→重新生效→立即重算）。不对称设计有意：批量只给"听源头"方向，批量坚持会废掉报警。接口 `/api/adjust/expired/revoke_all`、`/api/adjust/{id}/rearm`（均要会话）。测试 136（admin_edit 24→28）。**R2~R4 暂缓**（2026-07-11 复核后建议、明昊未再推进：现有每日备份+过期疑似/missing 报警够用，等真实运维痛点触发再启，计划存 `4_管理过程/12_R2R4改造施工与测试计划_20260711.md`）。
...
- 测试/版本/迭代 → `软件工程文档/3_测试/05_测试说明`、`4_管理过程/CHANGELOG.md` + `16_迭代计划_BU分页.md`
