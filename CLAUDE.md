...
| `bu.py` | BU 数据归属读写/校验（BU 名+负责人+销售名单；「整体」保留字；无密码字段；真实配置只在 `数据/BU配置.json`）|
| `render.py` | 整体用户端与 BU 独立页 HTML（含自改密码弹窗）| `charts/theme/assets` 渲染件 |
| `server.py` | FastAPI 出口：`/`=看板统一入口（账号权限分流）、`/bu/{BU名}`、管理员`/admin`（账号+密码·无身份下拉）、设置页账号与权限卡、`/api/accounts` `/api/my_passwd`、控制台HTML |
