...
## 当前状态（2.2.4 · 2026-07-21）

- **版本**：`VERSION` / `src/version.py` = **2.2.4**（陆总 07-21 十项：时间左移+导出恢复+毛利率卡+回款改名/基准线+公共费用÷100+装修归固定运营+门槛解耦）。生产可回滚上一 commit。
- **看端**：时间选择器左上（标题旁）；顶栏「⬇ 导出」→ 服务端 PNG；KPI「毛利率」大数字=%；回款卡名「下单/回款情况」、y 轴覆盖月均预算；费用三视图含手填房租/物业/装修 +「人工分摊(公共)」组；装修归固定运营；无智云/台账配置可进空态/最后数据（鉴权保留）。
- **管理端**：退出在设置页底；公共费用总额显示元（分÷100）；分摊/目标/三类手填同前。
- **工程**：`sh tests/run_verify.sh` 判绿；`tests/test_task_2_2_4.py` + `test_expense_zhuangxiu_alloc.py`。
- **部署**：Ubuntu 唯一主线；nginx 发 dist + 反代；运维见 `docs/Runbook.md` §0。
- **红线**：核心 total/pretax/收入/成本 零未授权 diff；32 周期回归；手填未填=0。
