...
```
- **当前状态（v2.0.0-beta · 2026-07-18 · 任务书54.2）**：主部署线 **Ubuntu 26.04 唯一**（系统 python3≥3.12；nginx 发 dist + 反代 API；Windows `.bat`/schtasks 已删）。看端 Vue + **SciFi kit 深空指挥舱气质** + ECharts（对照 legacy 定格基准）；亮暗纯前端；费用月度=多系列发光折线；KPI 五卡/环形底图例/回款双柱/金黄毛利率线。口径配置 UI/API 下线。明细**白名单列**；登录明文+踢会话。**库内金额 INTEGER 分**；WAL。算账/32 周期红线未改。legacy 壳保留至 55。tag **`stage54p2` 未 push**。
