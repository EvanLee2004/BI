...
6. **发布安全**：分支已理成单主分支（2026-07-09）——**`main`=唯一主分支**（开发+推 GitHub 都在它上）；`archive-本机完整历史-禁推`=旧 master 改名的只读历史档案（**历史含真实数据**，pushRemote=no_push 永不推，tags v1.0~v7.0 挂在它上**绝不 push --tags**）；**推公开仓库前肉眼核 manifest：真实金额/客户名/账号密码/cookie/收单台账共享盘路径绝不进库**。⚠口径更新（2026-07-13 明昊拍板）：**智云内网 base_url + app_id + 四表 worksheetId 允许进公开库**（`fetch_zhiyun.ZHIYUN_DEFAULTS` 内置默认，部署免拷模板），不再当违禁品。
