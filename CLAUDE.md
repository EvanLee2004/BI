...
| `profit.py` | **只吃库**算 summary（纯函数，不再扫文件）；`filter_rows_by_sales`(BU 按销售过滤) + `compute_unassigned_orders_by_period`(A3 每周期未归属下单额)|
...
| `server.py` | FastAPI 出口：`/`=看板统一入口（账号权限分流·`_main_with_nav` 注入 BU 入口条+A3 未归属提示）、`/bu/{BU名}`、管理员`/admin`、`/api/accounts` `/api/my_passwd` `/api/sales_pool`(A1) `/api/config_changes`(C3)、控制台HTML；C3 留痕 diff 与 `_audit` 在各配置写接口就地脱敏落库 |
...
- **v8.2 板块③按天时段常显跟顶 + 公共费用分摊（迭代 17·A 改口 + A2）已完成**：**批次A** 顶部「看哪段」不动；板块③日期区默认展开（无 dailyBtn）、起止默认跟顶（初始全年）、查询才 `/api/daily`、**返回默认（年）** 回全年预渲染；BU 页仍禁 daily（铁律12）。**批次B** `BU配置.json` 增加 `公共费用分摊启用`（默认 false）+ 每 BU `分摊比例`；开时台账 5 类×比例注入 `build_bu_summary`/`apply_public_expense_allocation`（守恒、手填不摊、附加税按 BU 收入）；管理端归属卡下分摊 UI；C3 类别「分摊」。测试 **223→227** 全绿+回归红线 32 周期；8020 探针通。
