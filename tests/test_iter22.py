...
class TestSystemCostRate(unittest.TestCase):
    ...
    def test_sales_card_hides_rate(self):
        period = {"range": ("2026-01-01", "2026-12-31"), "profit_rankings": {
...
class TestSalesProfitMarginRow(unittest.TestCase):
    def _period(self):
        ...
        return {"revenue_net": 1000.0, "production_cost": 400.0, "gross_profit": 600.0,
                "system_direct_cost": 400.0, "inhouse_cost": 0.0,
                "surtax": 7.2, "other_pl": 0.0, "pretax_profit": 500.0,
                "pretax_margin_pct": 50.0, "range": ("2026-01-01", "2026-12-31"),
                "expense": {"营销费用": 10.0, "管理费用": 20.0, "固定运营费用": 30.0,
                            "研发费用": 0.0, "财务费用": 0.0, "total": 60.0},
                "manual": {k: 0.0 for k in ("营销人力成本", "管理人力成本", "研发人力成本", "财务费用补充",
...
class TestOrdersByBU(unittest.TestCase):
    ...
    def test_ranking_with_name_of(self):
        rows = [{"下单日期": "2026-02-01", "预估金额": 100.0, "销售": "张三"},
                {"下单日期": "2026-03-01", "预估金额": 50.0, "销售": "李四"},
                {"下单日期": "2026-03-02", "预估金额": 30.0, "销售": "王麻子"}]  # 未归属
        m = {"张三": "游戏", "李四": "游戏"}
        rk = profit.compute_ranking(rows, "销售", "预估金额", "下单日期", S, E,
                                    empty_label="（未归属）",
...
class TestBuPageAlignment(unittest.TestCase):
    ...
    def _fine(self):
        return {"管理费用": [("办公费", 300.0), ("差旅费", 100.0)],
                "市场费用": [("差旅费", 50.0)]}
    ...
    def _bu_period(self, alloc_added=None):
        ...
        led["管理费用"] = 400.0
        p = {"revenue_net": 1000.0, "production_cost": 400.0, "gross_profit": 600.0,
             "system_direct_cost": 400.0, "inhouse_cost": 0.0,
             "surtax": 7.2, "other_pl": 0.0, "pretax_profit": 192.8,
             "pretax_margin_pct": 19.3, "range": ("2026-01-01", "2026-12-31"),
             "expense": {"营销费用": 0.0, "管理费用": 400.0, "固定运营费用": 0.0,
                         "研发费用": 0.0, "财务费用": 0.0, "total": 400.0},
             "manual": {k: 0.0 for k in ("营销人力成本", "管理人力成本", "研发人力成本", "财务费用补充",
                                         "PM人力成本", "VM人力成本", "实际内部译员成本", "税费损失",
                                         "技术流量成本", "其他（生产成本）", "其他损益")},
             "ledger_expenses": led}
    ...
    def test_bu_pl_alloc_decomposed(self):
        p = self._bu_period(alloc_added={"管理费用": 100.0, "市场费用": 0.0,
                                         "固定运营费用": 0.0, "技术服务费": 0.0, "财务费用": 0.0})
...
class TestCostVatManual(unittest.TestCase):
    ...
    def _summary(self, manual_raw):
        ...
        return profit.build_summary(self.cfg, [], [], [], [], header, [], 2026,
                                    self.today, manual_raw=manual_raw)
