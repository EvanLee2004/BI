...
> 产品现网以根 **`VERSION`（当前 3.7.15）** 为准 · 配图：`docs/设计图/04_部署与运行拓扑图.svg`（候选 :8019 见 CHANGELOG 3.7.3）  
> **主线 Ubuntu**（systemd kanban + nginx:80 + ScheduleLoop + cron 体检/备份 + **CIFS `/mnt/kanban-ledger`**）；Windows 线已退役。  
> 变更史见 `CHANGELOG.md`；运维实况见 `Runbook.md` §0。手填重算跳过 std 重建（MADR-0023）。台账管理端 B 见 3.7.15。
...
| 登录/错误页 HTML | `view_login.html`（全员登录）；`admin_login.html` 仅为跳转壳→`/login`；错误页模板 | 鉴权与异常页 |
