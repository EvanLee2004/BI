...
> 产品现网以根 **`VERSION`（当前 3.7.7）** 为准 · 配图：`docs/设计图/04_部署与运行拓扑图.svg` + `docs/images/deploy.png`（候选 :8019 见 CHANGELOG 3.7.3）  
> **主线 Ubuntu**（systemd kanban + nginx:80 + 进程内 ScheduleLoop + cron 体检/备份）；Windows 线已退役。  
> 变更史见 `CHANGELOG.md`；运维实况见 `Runbook.md` §0。手填重算跳过 std 重建（MADR-0023）。
