...
| ~~GET ~~ | 2.6.9 S8 已删 |
| GET | `/api/settings` | 管理员：含 `run_log_keep_days` / `disk_free_min_ratio` 等（**无**飞书 webhook 字段） |
| POST | `/api/settings` | 管理员：写本地配置覆盖层（**不接受**飞书 webhook） |
...
| GET/POST | `/api/budget` · `/api/budget_batch` | 业绩目标（`/api/budget_depts` 2.6.9 已删） |
