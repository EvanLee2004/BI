...
> ⚠ **历史教学快照 · 非现网架构真相**。  
> **现网**：根 `VERSION`（**3.7.7**）+ `Agent.md` / `README.md` / 项目 `progress.md` 顶部（Domain→VM→Vue · `/api/v1/*` · 仅 `kanban_sid` · 候选预热发版）。  
> **读者**：明昊。正文仍按 v1.5 分离时代路径讲概念；路径/部署以现网为准。  
> **写法**：大白话 + 本系统例子 · mermaid。**不写真实金额/客户/密码**。
...
代码：`accounts.can_see_bu` / `bu_names_of`；`server` 里 `/api/v1/daily` 对 BU 会话 401。
...
| 词 | 大白话 | 本系统例子 |
|----|--------|------------|
| HTTP | 浏览器与服务器要数据的规矩 | `GET /api/v1/health` |
| 路由 | URL 路径对应哪个处理函数 | `/bu/游戏` |
| 会话/cookie | 登录后的通行证 | `kanban_sid`（2.6.0；旧名兼容读 21 天） |
