...
| 观感 | — | 搬家不装修 | 同左 |
| 回退 | — | `KANBAN_LEGACY_INLINE=1` | 同一开关语义 |
...
整体页登录后，可能先拿到很轻的 `static/shell.html`，它再 `fetch('/api/v1/cockpit/view')` 把真正驾驶舱 HTML 灌进去——**像先拿信封再装信**。BU 页为隔离仍常直接出 HTML。
...
16. `KANBAN_LEGACY_INLINE=1` 对 /admin 有何影响？  
