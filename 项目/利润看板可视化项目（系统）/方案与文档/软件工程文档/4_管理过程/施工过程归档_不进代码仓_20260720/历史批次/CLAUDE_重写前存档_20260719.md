...
```
① 抓数层(采集)  智云四源自动抓(默认开) + 台账 SMB + 管理端表单
   └▶ 〔进料口〕 数据/ ——抓数层↔清洗层的接缝
        └▶ ② 清洗层  ingest
             └▶ ③ SQLite(看板.db)
                  └▶ ④ profit → summary
...
```
- **当前状态（v2.0.0-beta · 2026-07-18 · 任务书54.3）**：主部署线 **Ubuntu 26.04 唯一**（系统 python3≥3.12；nginx 发 dist + 反代 API；Windows `.bat`/schtasks 已删）。看端 Vue + **SciFi kit 深空指挥舱气质** + ECharts（对照 legacy 定格基准）；亮暗纯前端；费用月度=多系列发光折线（⚠与 legacy 面积图冲突·**待明昊拍板** B06）；KPI 五卡/环形底图例/**回款双柱+金黄回款率线**/金黄毛利率线。**B-01 按时间段查询=原位切区间双卡（回款总图不动、版面不跳、返回默认恢复）**。口径配置 UI/API 下线。明细**白名单列**；登录明文+踢会话。**库内金额 INTEGER 分**；WAL。算账/32 周期红线未改。legacy 壳保留至 55。回款月均预算线/累计摘要条待后端填 VM 空字段。tag **`stage54p3` 未 push**。
...
python run.py --scheduled# 供 Linux cron 调用（deploy/linux/register_schedule.sh）
sh tests/run_verify.sh   # 一键验证：语法+端到端+回归红线+全部测试
```
- **Linux 部署（唯一主线·任务书54）**：`docs/Ubuntu部署手册.md`；`systemctl enable --now kanban`；`bash deploy/linux/register_schedule.sh`；看门狗=`deploy/linux/start_with_rollback.sh`（退出码 42 + `.update_rollback`）。
- **Windows 已退役（任务书54 明昊拍板）**：根目录 `*.bat` / schtasks / Windows 部署手册已删；勿再加回。
...
| 文件 | 职责 |
|---|---|
| `schema.py` | **全表 DDL + SCHEMA_VERSION + 金额分迁移**；可调整字段/归属月字段唯一源；`meta_schema` |
| `money.py` | **元↔分**（Decimal ROUND_HALF_UP）；std/manual 金额列映射 |
| `db.py` | 连接（WAL/busy_timeout）+ 读回层（分→元 float）+ 明细查询 + 写(调整/手填/预算·元→分) + 定位键重复审计 + quick_check |
| `ingest/` | 更新管道：`readers` `normalize`(元算定位键) `fetch`/`login_zhiyun`/`fetch_zhiyun` `migrate` `adjust`(分↔元) `archive`(备份+**restore**) `__init__`(**单事务重建**+dup 审计+quick_check) |
...
| `version.py` | **产品版本号 + 面向用户更新日志**：读根目录 `VERSION`（现 `1.6.0-beta`）；`product_stage`/`product_label`/`changelog`；产品号≠git 开发号 |
| `updater.py` | **一键更新 + 看门狗**：`check_update`/`apply_update`→ff-only→依赖同步→`.update_rollback`；退出码 42；Linux 看门狗=`deploy/linux/start_with_rollback.sh`（Windows 载体已删） |
...
## 完美主义（2026-07-18 明昊拍板·施工/验收 AI 必守）

- **生产封板 = PERF 全量**：见 `方案与文档/0_规划与调研/20260718_变更迈进计划书_完美生产L2.md`（唯一 Active Roadmap）。
- **禁止**「不做管理端 Vue / 巨石不拆 / 差不多先上」；根 `CLAUDE.md` #11。
- 批次 B0–B7 可排序，**DoD 不可打折**。

## 铁律（违反就出错，必守）

1. **智云导出 xlsx 绝不 `read_only=True`**（`<dimension>` 不可信，老坑）；**列按表头找、不按位置**；**必需列缺失即报错，不静默算 0**。
2. **前端不做金额运算**（tests 守卫）；求和/口径都在 profit。
2b. **看端费用明细列走白名单**（任务书41·D，2026-07-16）：展示列= `VIEW_EXPENSE_COLUMNS` 固定顺序；**口径人=业务员**（花钱的人），提单人/定位键/收单月份/归属月等看端绝不出现。管理端「数据调整」仍全列。导出跟随看端白名单。归属月计算仍靠库内「收单月份」——只动展示层。
2c. **平台=Linux 单线（任务书54 明昊拍板）**：定时同步走 `server.sync_schedule` → **仅** Linux crontab 哨兵（非 linux no-op）；看门狗仅 `deploy/linux/start_with_rollback.sh`（退出码 42 + `.update_rollback` 自愈保留）。**禁止**再引入 `.bat` / schtasks / win32 分支。不引 Docker。
2d. **业务层零裸 SQL**（任务书43）：`SELECT/INSERT/UPDATE/DELETE/PRAGMA` 字面量只许出现在 `db.py` / `db_write.py` / `schema.py`；profit/routes/ingest 走函数入口。守卫 `test_task43_arch`。
2e. **告警失败不影响主流程**（任务书43）：飞书 webhook 空=静默；超时≤3s；任何异常吞掉。
2f. **共享盘只读**（任务书43·明昊拍板）：`ledger_share_path`/CIFS 只允许 `fetch` 读后 `copy2` 到本地；绝不回写共享盘（守卫测试）。
3. **回归红线**：改动后页面所有数字须与基线一分不差——动 loaders/计算前先跑 `tests/regress_db_vs_files.py`（从库算==从文件算，32 周期）。
...
18. **一键更新只认 fast-forward + 护栏 + 依赖同步 + 自动回滚（v8.7·v8.7.3 扩）**：`updater.apply_update` 只跑 `git pull --ff-only`（绝不合并/变基/产生冲突）；工作区脏、本地与远端分叉、非 git 仓库→一律拒绝并提示人工，绝不强拉覆盖。**拉取后依赖自动同步**：`requirements.txt` 变了就用当前解释器 `pip install`（`_run_pip`，装进同一 venv）；**装失败→`git reset --hard` 回滚这次拉取、返回 ok=False 不重启**（更新期自愈）。**成功则写回滚点标记 `.update_rollback`（更新前 commit，gitignore）**；重启靠看门狗——拉取成功后进程以退出码 `RESTART_EXIT_CODE=42` 退出，`deploy/linux/start_with_rollback.sh` 据码用新代码重拉起（42 两处必须一致）。**坏版本自愈**：看门狗遇非 42 崩溃时若 `.update_rollback` 仍在=更新后启动即崩→自动 `git reset --hard <标记commit>` 回滚一次再起（只一次，删标记后再崩走 5 次停下报警）；服务正常起 20s 后 `server.serve` 调 `clear_rollback_marker` 清标记=确认这版没崩。改更新逻辑别绕过护栏、别在无看门狗时指望自动重启，`.update_rollback` 写/清/回滚三处语义要对齐。

20. **验证只认真实退出码，绝不套 `tail`/`head` 管道判绿（v1.2.2·2026-07-14 踩坑）**：`run_verify.sh` 有 `set -e`（任一测试非 0 立即停），但 `sh tests/run_verify.sh 2>&1 | tail -60` 这类调用会让整条命令的退出码变成**管道最后一环 `tail` 的 0**，把测试失败悄悄吞掉；且 `set -e` 会停在**第一个失败的文件**，其后的测试文件在这次运行里根本没跑到（一次带红推上远端就是这么发生的）。**改测试/代码后判绿，务必**：① 不接 `| tail`/`| head`，直接看脚本退出码（`sh tests/run_verify.sh; echo $?`）；② 要看尾部日志就先重定向到文件再 `tail` 那个文件（`… > /tmp/v.log 2>&1; echo $?`）；③ 逐文件独立跑一遍收集**全部**失败（`for f in tests/test_*.py; do python "$f" || echo FAIL $f; done`），别被 `set -e` 提前截断误判"其余都绿"。

21. **std 重建必须单事务（任务书33·A1）**：`BEGIN IMMEDIATE` → 清 std + 全量 INSERT → **一次** COMMIT；中途失败 ROLLBACK，旧数据完整。禁止清表后单独 commit 再插（崩溃=看板全零）。

22. **金额一律整数分，显示层才转格式（任务书33·A3）**：库内 INTEGER **分**；进料 `Decimal` 四舍五入；`profit` **全程分整数**，仅 `fmt_wan` 等显示层分→万元串；**定位键哈希用元字符串**（normalize 先哈希再转分）。adj 金额 原值/新值库内**分 TEXT**（SCHEMA v3；列表展示元）；预算比率走百分位点、**不** `yuan_to_fen`。禁止 float×100。
...
## 当前状态（2026-07-18 · 任务书54.3 像素+行为复刻）

- **阶段0**：Playwright 逐屏+逐交互实录 legacy（8019）→ `原始素材/…/interactions/`（仓库外·真实数据）+ `复刻清单.md`（进度真相表）。**踩坑固化**：legacy 周期面板 `#ppanel[hidden]` + document 外部点击自动关闭 → 抓交互须**每组从新加载页面重置状态**、force 点击、避开隐藏元素滚动，否则级联超时。
- **阶段2 · B-01（点名必修）已达标**：查询后回款总图不动、排名双卡原位切区间并标注、三态页高恒等无跳动、返回默认恢复——对齐 legacy 实录。修 `cockpit.ts`(daily 状态)+`DailyQuery.vue`(收敛控件·删自渲染双卡)+`RankingsDual.vue`(原位消费)。
- **阶段1 · 回款卡金线已达标**：`ReceiptsCard.vue` 加右轴金黄回款率线（现有 VM `ratio_pct_disp`·前端零运算）。
- **踩坑固化（改前端结构必核守卫）**：`test_parity_structure` 断言 RankingsDual 原始 src 含**静态** `data-source="rankings_view"`（勿改成 `:data-source` 动态绑定）；`test_task54p1_visual.test_dual_rank_shared` 原断言 DailyQuery 含 `dualRankBarOption`——B-01 收敛后已更新为 `setDaily`/`dailyDual` 新不变式。改 .vue 结构前先 grep tests 里对该组件的字面量断言。
- **未完成**：阶段1 其余逐卡/亮色/375/BU、阶段2 其余交互、阶段3 减重；回款月均预算线+累计摘要条**需后端填 VM 空字段**（`receipts_budget`/`summary_by_period`）；**费用月度图型面积 vs 折线待明昊拍板**。全绿 EXIT:0，tag `stage54p3` 未 push。交付报告 `docs/历史批次/20260718_任务书54.3交付报告.md`。

## 当前状态（2026-07-17 · 任务书50）

- **任务书50 真组件化 + ECharts + nginx 标准架构 + 登录回退（2026-07-17）**：删 SSO；明文密码+👁+改密踢会话；Vue `v-html` 清零（结构化 VM + 真事件）；`/api/v1/vm/ledger` 任何会话白名单；图表轴/中心后端串；nginx 发 dist 为生产标准（MADR-0009）；VERSION 2.0.0-beta。交付 `docs/历史批次/20260717_任务书50交付报告.md`。**main 只 commit 不 push**；32 周期数字红线不变。
- **铁律延伸（任务书50）**：① frontend/src 禁止 `v-html`（自由文本走文本插值/textContent）；② 看端明细任何会话（含管理员）一律白名单列，管理端数据调整接口另计；③ 验证只认真实退出码（铁律20）。

## 当前状态（2026-07-16 · 历史）


- **任务书39 看端布局统一与联动（2026-07-16·明昊二轮走查）**：A 回款图整行+图下摘要条；B 按时间段上移+全局周期联动；C 查询双血条（`/api/daily.dual_rankings` + BU `/api/bu_daily`）；D 费用明细胶囊/sticky/分页/年·月联动；E 费用月度堆叠柱 overall/BU（工资默隐并入其他）；F 黄横幅仅管理端；G 进度>999% 钳制。测试 `test_task39`。交付 `docs/历史批次/20260716_任务书39交付报告.md`。**main 只 commit 不 push**；32 周期数字红线不变。
- **任务书37 功能与 UI 收口（2026-07-16·明昊走查）**：A1–A6 UI（去刷新提示/双血条图例/50/50/周期标注/sticky/H1 行序）+ B7 六表列筛（`db.query_detail` filters + 管理端表头）+ B8 整体费用明细与工资默隐（`overall_see_salary` 覆盖层）+ B9/B10 抓数降级黄横幅与未来日体检。测试 `test_task37_*.py`。交付 `docs/历史批次/20260716_任务书37交付报告.md`。
- **任务书33 数据库大修 + 隐患清扫（2026-07-16）**：A1 原子重建 · A2 WAL · A3 金额分（SCHEMA v3：adj 分文本 + 预算比率百分位点）· A4 重复定位键 · A5 10 万行基准 · A6 备份恢复 · A7 quick_check · B 吞异常/config · C 文档。**前端/口径/golden 零变化**。交付报告 `docs/历史批次/20260716_任务书33交付报告.md`。规模基线 `docs/bench_scale_100k_latest.json`。
- **v1.3.1-beta 修去税两处活体 bug + 业绩目标跟顶部统一范围（2026-07-14·明昊实测·Claude修）**：v1.3.0 推完明昊本地重启实测揪出两个**单测/接口测不出、只有驱动真实浏览器才现形**的坑——**BUG-A**：`server.py` 缺 `import charts`→`_detax_payload` 真实数据下 500（空库单测漏网，类别空时那行不执行；43 类才触发）；修=函数内局部 import + 测试补种真实台账行。**BUG-B**：去税表 id `dTbl` 与「明细编辑」主表 `dTbl` 撞车→`getElementById` 命中错表（在 `tbl-box lg wrap` 折叠容器 height:0），43 行去税数据灌进错表、真去税表空白（接口/单测/模板grep全被撞车的那个骗过）；修=去税表改 `dxTbl` + **新增全控制台重复 id 扫描单测**。**另**：业绩目标块删自带 `bY/bScope` 独立下拉、改跟顶部 `mY/mScope`（`bLoad` 重写·`test_budget` 加 `TestBudgetFollowsTopFilter` 守卫）。**活体复验**：去税表 43 类降序渲染（`offsetHeight≈1990`·房租264.2万置顶）、明细表51行不污染、业绩目标无独立下拉。**教训固化**：新渲染出口必须驱动真实页面看 DOM（`read_page`+`offsetHeight`），别信单测/接口/grep 假绿。全绿+红线32周期不变，VERSION→1.3.1-beta。
- **v1.3.0-beta 费用去税率手填（陆总0714·E2）+ 视图档案（完整/精简·另一账号施工）**（commit `dc610e6` 已双推 GitHub+Gitee）：
  - **费用去税率**：台账费用含税→按类别手填增值税率%还原不含税（`不含税=含税÷(1+率)`）。陆总要「留空让填、默认不去税、主要改房租/物业」。`profit.detax_ledger_rows` 在 `build_summary` 源头统一去税（**空=恒等·数字不变·回归红线中性**·源头去一次→大类/细类/按BU/公共池全一致无两处真相）；`build_summary`/`build_bu_summary` 加 `detax_rates` 参数，`core.summary_from_conn` 从库装载。新表 `manual_费用去税率(费用类别 PK,税率,...)` + `db.set_detax_rate`（0~100·None/0=删行）/`load_detax_rates`/`list_detax_categories`（白名单大类下细类+全年金额降序）。管理端人工填写页「💧 费用去税率」区块 + `GET/POST /api/detax_rates`（管理员·C3「去税」·保存即重算）。**全局一套·常年沿用**（明昊拍板不按月，区别于分摊比例按月）。`test_detax.py` 12 例。
  - **精简视图已下线（2026-07-15 收口）**：profile 管道删除；展示 `render` + `render_shell` + `render_widgets`；shell 由 `SERVE_SHELL` 控制；`test_profile` 仅守卫无 executive/data-profile。
  - 两特性 server.py 改动交织无法按文件拆分，明昊拍板一起提交（视图档案属路线图 Phase1）→ commit `dc610e6` 双推 GitHub+Gitee。⚠随后活体测出去税两处 bug，见上条 v1.3.1。
- **v1.2.2-beta 看端领导向精简 + KPI 扫读改版 + 回款/排名全站按 BU + 管理端亮色主题（2026-07-14·明昊拍板·Claude施工）**：v1.2.1 后连续一批看端优化（12 commit `848f275`→`1025c06`）：① 看端文案精简、利润表抽屉金额改绝对值（行名已带加/减语义）、去「管理毛利（完整）」「（台账）」字样、BU 子页去口径长文；② KPI 五卡标当前时段、去迷你折线改显「全年峰值」、交付卡补「交付占下单」、回款卡补「已交付未回款」；③ 按时段查询首卡统一按 BU、**BU 页回款卡随周期切**（`.pv` 预渲染）、用户端排名卡隐藏「（未填）」行（后端 unfilled 仍算·守恒）、排名「其余」用 `full_items` 本地弹窗（不向 BU 页开 `/api/daily`·`/api/profit_ranking`·**铁律12**）；④ **管理端亮色主题 + 全局深浅色同步**（`cockpit-theme` localStorage+跨窗 postMessage）、浅色回款驾驶舱文字对比度加深；⑤ 期间费用环形图放大（size 190→300·卡框高不变）。**合计 382 例全绿 + 回归红线 32 周期一分不差**（页面数字不动）。⚠ 本版收尾发现两处旧断言未随删文案同步（`test_profit_ranking` 断言「（未填）」、`test_iter22` 断言「台账直记」），且 `run_verify.sh | tail` 管道曾把 `set -e` 退出码吞成 0 掩盖假绿——已修断言并立铁律：**验证一律看真实退出码、不套 `tail` 管道**（见铁律20 / `11_bug台账`）。
- **v1.2.1-beta 修手填清单硬编码 + 看端提示精简（2026-07-14·grok施工）**：管理端手填项目清单从 config.manual_items 注入（`__MANUAL_ITEMS__` 占位符·曾硬编码致「直接成本增值税」无填数入口=code review 隐患#4修复）；看端隐藏「待分类费用口径提示」「未归属BU」文案（管理端体检保留）；BU 页去重复返回按钮。test_iter22 +2。
- **迭代22：v1.2.0-beta 陆总0714过盘反馈全量落地（2026-07-14）**：A小改（回款率%挪线点/删防误读长句/去峰谷徽章/「项目毛利率」→「系统成本率」且按销售卡不显示率/生产毛利趋势改名/浅色灰阶加深）；B 分摊比例**沿用最近填写月**（`db.effective_alloc_month/ratios`·填0=显式不分摊·保存固化生效全集·管理端显"沿用X月"）+利润表「销售利润率」行；C 下单卡三大BU进度（`core.attach_bu_orders`·只挂整体页）+「下单·按BU」排名（`compute_ranking` 新 `name_of`·未配BU回退部门）；D BU页对齐整体页（趋势图/费用类别视图 `_fine_to_rows`/收入毛利结构/费用下钻含「分摊自公共」`alloc_added`/回款卡/导出 `/bu/{name}/export.png` 会话闸）；E1 手填「直接成本增值税」（默认0·生产成本减项）。**test_iter22 24例新增，run_verify 全绿+红线32周期不变**。E2除税系数挂起；AI问答/通用导出明确不做。
- **迭代21：v1.1.1-beta 回款卡月份高亮联动 + 月周期待分类提示 + 台账归属中心未知名校验（2026-07-14）**：① 回款图全年视角不变，切周期只高亮对应月（`data-rm`/`data-rm-map`/CSS `rm-dim`·`rm-on`，前端零运算）。② 有未分类费用时月/季也出无金额口径提示。③ 白名单费用行归属中心未知名→`health.warnings` + 设置页 BU 卡黄条；无 BU 配置跳过。`test_cockpit` +3、`test_iter21` 7 例；数字红线不动。
- **迭代20：v1.0.6 回款板块下移 + v1.1.0 公共费用按月分摊（2026-07-14）**：①板块④改「资金与回款」=回款情况卡（自②下移）+下单/回款排名；rc-body 改 column 修图例错位。②分摊改按月：新表 `manual_分摊比例`（月+BU 主键）、`db.set/get/load_alloc_ratios`、`profit.apply_public_expense_allocation_monthly`（逐月公共池×比例，可部分分摊残留留公司层）+`alloc_amounts_by_period`+`apply_alloc_to_pc_view`（全公司「构成·按业务BU」跟着挪：公共=残留+「已分摊至各BU」负行、BU=直记+「分摊自公共」行，**各条合计不变**）；`core.alloc_context`/`attach_allocation_to_summary`（主 summary 与 BU 页共用同一套结果·孤儿 BU 比例体检黄）。管理端：人工填写页「公共费用分摊比例（按月）」区块（本月公共费用总额后端下发·合计>100 拒·并入底部保存）、`GET/POST /api/alloc_ratios`（管理员·C3「分摊」）、设置页旧全年比例区下线（buAllocLegacy 提示）。**旧 `BU配置.json` 静态比例停用不再消费**（bu.py 读写保留兼容旧测试）。`test_alloc_monthly` 16 例新增。
- **v1.0.5-beta 一键更新装依赖走国内 pip 镜像（2026-07-14）**：config.json 出厂默认 `pip_mirror`（清华 PyPI 镜像·完整同步源），`updater._run_pip` 读 `loaders.load_config` 非空加 `-i <镜像>`；空串/缺字段/config 读不到=不加（走 pip 自身配置，应急后门，读配置失败不挡装依赖）；换镜像走 `数据/本地配置.json` 覆盖层（铁律19，别改 config.json）。只动 _run_pip 这一步，check/apply/看门狗/回滚不碰。`test_update` +5（TestRunPipMirror）。
- **v1.0.4-beta 更新诚实化 + 设置统一保存 + 回款卡对称 + 小字放大（2026-07-13 深夜·明昊反馈）**：①「更新数据」完成后拉 `/api/health`，绿且无告警才报成功，否则「更新完成，但有 N 个问题 · 点击查看」toast（warn/err 级、可点击展开体检明细，`refreshResultToast`/`showToast(t,cls,onclick)`）；②设置页删五个就近保存按钮→统一底部 `.save-bar`（`setDirty` 标脏 + `setSaveAll` 汇总，各 save 函数返回 true/false）；③`.rc-split` 改 1fr|1fr 对称；④theme 新增 `--note` 色、口径小字全面 +1~1.5px。**修回归红线假红**：手填库为唯一源、旧 xlsx 不回写→管理端改手填即假红；`regress_db_vs_files.summary_from_files` 两路共用库 manual（红线=只守 5 个文件源入库一致）。CHANGELOG 补记 v8.8~v1.0.3 五版欠账。
...
- 运行/打包/部署/发布流程 → 本目录 `README.md`、`docs/Ubuntu部署手册.md`
...
## 铁律17 补充（任务书52）
Vue 弹窗必须 `Teleport to="body"` + `.rkm-mask{position:fixed}`（theme.css 有守卫）；禁止 transform 祖先夹 fixed。
