...
def register(app, d):  # noqa: C901  # 纯路由/装配分发壳，复杂度在子 handler
    ...
    from routes._srv import start_refresh_async  # 任务书64·D9 共享 helper
    ...
    def api_health(request: Request):  # noqa: C901  # 2.6.3 合成黄/红/台账/账号/schedule
        ...
        # 2.2.8 方案 B：未归属 + 手填缺月等业务提醒 → 绿抬黄（不盖红）；与管道抓数红分离
        from audit_diff import apply_business_health_yellow
        ...
        # 任务书66·D：定位键重复等 → info（不驱动黄）
        info = list(body.get("info") or [])
        dups = body.get("duplicate_locators") or {}
        ...
        sched = {}
        try:
            from schedule_loop import health_messages_from_schedule, schedule_ledger

            sched = schedule_ledger(cfg, root)
            # 3.7.5：消息组装 SSOT = health_messages_from_schedule（upcoming 不入 reasons）
            sched_msgs, yellow_nudge = health_messages_from_schedule(sched)
            for msg in reversed(sched_msgs):
                if msg not in reasons:
                    reasons = [msg] + list(reasons)
            if yellow_nudge and result in ("绿", None):
                result = "黄"
        except Exception:
            pass
        # 2.6.3·B6：缺台账年页横幅
