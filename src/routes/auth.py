...
def register(app, d):  # noqa: C901  # 纯路由/装配分发壳，复杂度在子 handler
    ...
    def user_page(request: Request):
        ...
        if acc:
            ...
            if names:
                existing = [n for n in names if n in _bu_pages()]
                if not existing:
                    return RedirectResponse(
                        login_redirect.login_url(
                            msg="你绑定的 BU 已被管理员移除，请重新登录或联系管理员"
                        ),
                        status_code=303,
                    )
                # BU 账号：303 到本 BU（2.4.3 / 2.5.0）
                return RedirectResponse(
                    login_redirect.default_redirect_for_account(acc, bu_pages=_bu_pages()),
                    status_code=303,
                )
            if authz.is_admin(acc):
    ...
    def viewer_login_page(request: Request):
        """2.5.0：全员唯一登录页（看端壳）。已登录则按角色离开登录页。"""
        if _user(request):
            return RedirectResponse("/admin", status_code=303)
        acc = _vacc_row(request)
        if acc:
    ...
    def api_v1_logout(request: Request):
        ...
        # 2.6.0：清 sid + 两旧名（path/samesite 与 set 一致）
