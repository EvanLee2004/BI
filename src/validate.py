...
def _scan_zhiyun_sheet(rep: Report, source: str, ws, date_col: str, amount_col: str,
    ...
    if bad_date:
        rep.error(source, f"「{date_col}」列有日期解析不出的行：{_rows_desc(bad_date)}——这些行会被整条剔除、不计入任何周期")
    if bad_amt:
...
def _validate_ledger(rep: Report, cfg: dict, path: Path, year: int) -> None:
    ...
    if bad_date:
        rep.error(src, f"「收单日期/收单月份」都解析不出的行：{_rows_desc(bad_date)}——这些费用会无声消失（收单月份要填数字如 01，别填「1月」）")
    for cat, rs in sorted(bad_cat.items(), key=lambda x: -len(x[1])):
        rep.error(src, f"「对应报表大类」出现口径外的值「{cat}」：{_rows_desc(rs)}——不在 8 大类里（可能是错别字），这些费用不会计入任何科目")
...
def _validate_manual(rep: Report, cfg: dict, path: Path, year: int) -> None:
    ...
    if unknown:
        rep.warn(src, f"「项目」列有 config 里不认识的名字：{'、'.join(unknown[:MAX_ROWS_SHOWN])}——这些行不会被读取（改名或在 config.manual_items 里登记）")
    if bad_val:
...
def validate_all(cfg: dict, year: int, root: Path | None = None) -> Report:
    ...
    if pdp is not None and pdp.suffix.lower() == ".csv":
        ...
        if ws is not None:
            _scan_zhiyun_sheet(rep, "项目明细(智云)", ws, cc["project_delivery_date"], cc["project_revenue"],
                               [cc["project_delivery_date"], cc["project_revenue"], cc["project_cost"]])

    ws = _open_sheet(rep, "下单(智云)", base / cfg["files"]["orders"], "下单")
    if ws is not None:
        _scan_zhiyun_sheet(rep, "下单(智云)", ws, cc["order_date"], cc["order_amount"],
                           [cc["order_date"], cc["order_amount"]])

    ws = _open_sheet(rep, "回款记录(智云)", base / cfg["files"]["receipts"], "回款记录")
    if ws is not None:
        _scan_zhiyun_sheet(rep, "回款记录(智云)", ws, cc["receipt_date"], cc["receipt_amount"],
                           [cc["receipt_date"], cc["receipt_amount"]])

    ws = _open_sheet(rep, "内部译员(智云)", base / cfg["files"]["inhouse"], "任务")
    if ws is not None:
        _scan_zhiyun_sheet(rep, "内部译员(智云)", ws, cc["inhouse_date"], cc["inhouse_amount"],
