...
def _normalize_bus_entries(bus: list[dict]) -> list[dict]:
    """逐条规范化 BU；一人一 BU（先到先得）。"""
...
def _resolve_allocation_flags(out: list[dict]) -> bool:
    """全空=不分摊；有填=启用（须齐全且合计100%）。返回 enabled。"""
...
def save_bu_config(cfg: dict, root: Path | None, bus: list[dict], 公共费用分摊启用: bool = False) -> dict:
    ...
    **分摊**：各 BU 比例**全部为空 → 不分摊**（忽略开关）；任一有值 → 启用，
    ...
    # 参数保留兼容旧调用方（分摊实际由比例齐备性决定）
    _ = 公共费用分摊启用
    out = _normalize_bus_entries(bus)
    enabled = _resolve_allocation_flags(out)
