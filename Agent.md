...
- **版本**：读根目录 **`VERSION`** + 项目 `progress.md` 顶部（文档收口时 **3.7.16**；3.7.15 台账 **CIFS 可配置 B**；3.7.14 审计加固 + 生产 CIFS 挂载；候选预热发版默认对齐 version/commit；首屏六段 + KC；API v1 + `kanban_sid`）。历史：3.7.x 能力矩阵/PM/费用视图；3.6.x 六段/KC/CSRF；3.0.0 去 render 双轨。  
...
7. **收入毛利榜** `GET /api/v1/rankings/profit`（2.7.1 唯一路径）：未登录 401；已登录无权限 403；有 `bu` 须能看该 BU 且只返回本 BU 销售过滤行。**下单回款榜**另走 `/api/v1/rankings/full`，禁止合并。
...
5. **定时刷新（3.7.19）**：`schedule_times` 每个时点各自完整刷新；过点未 success 的槽从早到晚逐个补跑（每 tick 最多 1 次）；busy 排队不丢；**不以「漏跑」抬体检黄或发用户告警**。
...
10. **登录锁定按账号+IP**；密码非空即可，不强制长度/字符类型（2.6.12 明昊拍板；明文存储口径不变·MADR-0020）。
